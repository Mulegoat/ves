...
function closeMobileMenu(menuEl) {
    if (menuEl.className = menuEl.className.replace(/\btriggered\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className.replace(/\bmobile-show\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className + " mobile-hide", document.getElementById("home"))
        for (var homeEl = document.getElementById("home"), homeItems = homeEl.getElementsByClassName("item"), i = 0; i < homeItems.length; i++) homeItems[i].className = homeItems[i].className + " menu-closed", homeItems[i].className = homeItems[i].className.replace(/\bmenu-open\b/, "");
    ...
    if (document.getElementById("media")) {
        ...
        mediaEl.className = mediaEl.className + " menu-closed", mediaEl.className = mediaEl.className.replace(/\bmenu-open\b/, "")
    }
    menuEl.style.margin = "0"
}

function openMobileMenu(menuEl) {
    if (menuEl.className = menuEl.className + " triggered", document.getElementById("menu").className = document.getElementById("menu").className.replace(/\bmobile-hide\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className + " mobile-show", document.getElementById("home"))
        for (var homeEl = document.getElementById("home"), homeItems = homeEl.getElementsByClassName("item"), i = 0; i < homeItems.length; i++) homeItems[i].className = homeItems[i].className + " menu-open", homeItems[i].className = homeItems[i].className.replace(/\bmenu-closed\b/, "");
    if (document.getElementById("two-column")) {
        var twoColumnEl = document.getElementById("two-column"),
            moduleListEl = twoColumnEl.getElementsByClassName("module_list")[0];
...
function clickPage(e) {
    e.srcElement && ("share" !== e.srcElement.id && "menu-item share" !== e.srcElement.className && (document.getElementById("share").className = ""), "menu" !== e.srcElement.className && "trigger" !== e.srcElement.className && "box" !== e.srcElement.className && W() <= 1024 && closeMobileMenu(document.getElementById("mobile-menu")))
}
var traskindustries = angular.module("traskindustries", []);
traskindustries.config(["$routeProvider", "$locationProvider",
    function ($routes) {
        $routes.when("/home", {
            controller: "HomeController",
            templateUrl: "views/home.html"
        }), $routes.when("/innovation", {
            controller: "InnovationController",
            templateUrl: "views/two-column.html"
        }), $routes.when("/genetics", {
...
var isIE8 = -1 != navigator.userAgent.indexOf("MSIE 8"),
    ...
    setupListSpace = function () {
        ...
        List.addListener(Marq.goTo), Marq.addListener(List.goTo);
        ...
        for (var listVideos = $$(".video", $$(".module_list")), i = 0; i < listVideos.length; i++) {
            var parent = $$(".list_yt", listVideos[i]),
                targ = $$(".poster", parent);
            targ.src = getAttr(targ, "ref"), vine.bind($$(".icon-play", parent), "click", function (e) {
                var targ = e.srcElement || e.currentTarget || e,
                    _vidID = getAttr(targ.parentNode, "rel");
                new YTPlayerWrapper({
                    container: $$(".container", targ.parentNode).id,
                    vidID: _vidID
                }), targ.style.display = "none"
            })
        }
        for (var preloadImageSequence = function (path, total, ext, delay) {
            for (var i = 0; total + 1 > i; i++) staggerLoad(path + i + ext, i * delay)
            ...
            var t = listImageSeq[i] || listImageSeq,
                ...
                iO = {
                    fps: 10,
                    sequenceElement: container,
                    srcBase: sB,
                    totalFrames: fr,
                    spriteElement: $$(".icon", t),
                    autoplay: !1,
                    loop: !0
                };
        ...
        for (var mImageLoaded = function (e) {
            ...
            t.onload = mImageLoaded, t.src = getAttr(t, "ref")
        }
        for (var mVideos = $$(".yt_video", $$(".window", $$(".module_marquee"))), i = 0; i < mVideos.length; i++) {
            var targ = $$(".marquee_yt", mVideos[i]);
            targ.style.backgroundImage = "url('" + getAttr(targ, "ref") + "')", vine.bind($$(".play", targ), "click", function (e) {
                var targ = e.srcElement || e.currentTarget || e,
                    _vidID = getAttr(targ.parentNode, "rel");
                new YTPlayerWrapper({
                    container: $$(".container", targ.parentNode).id,
        ...
        for (var mHVideos = $$(".hosted_video", $$(".window", $$(".module_marquee"))), i = 0; i < mHVideos.length; i++) {
            var t = mHVideos[i] || mHVideos,
                c = $$(".content", t),
                sB = c.getAttribute ? c.getAttribute("ref") : c.attributes.ref.value,
                targ = $$(".h_video_container", t);
            new HostedVideoPlayer({
                srcBase: sB,
                container: targ,
                autoplay: "true",
                loop: "true"
        ...
        if (1 == $$(".module_list").length) {
            ...
            if (!isIE8) {
                var listInfos = $$(".infographic", $$(".module_list")),
                    iWidth = isIphone ? 605 : 655;
                console.log("iWidth", iWidth);
                for (var scl = listW / iWidth, i = 0; i < listInfos.length; i++) {
                    var t = listInfos[i] ? $$("iframe", listInfos[i]) : $$("iframe", listInfos),
                        iHeight = parseInt(getAttr(t, "ref")),
                        newHeight = scl * iHeight;
                    t.style["-webkit-transform"] = "scale(" + scl + ")", t.style["-ms-transform"] = "scale(" + scl + ")", t.style["-moz-transform"] = "scale(" + scl + ")", t.style.transform = "scale(" + scl + ")", t.parentNode.style.height = newHeight + "px", t.style.width = $$(".module_list").offsetWidth / scl + "px"
                }
            }
            var listImageSeq = $$(".img_sequence", $$(".module_list")),
                liW = 700;
            if (W() <= 641) var liH = 385;
            else if (W() <= 1023) var liH = 365;
            else var liH = 433;
        ...
        if (!result)
            for (var attrs = ele.attributes, length = attrs.length, i = 0; length > i; i++) attrs[i].nodeName === attr && (result = attrs[i].nodeValue);
        return result
    };
...
var animateIn = function () {
    -1 == $$("#menu").className.indexOf("inplace") && (TweenLite.to($$("#menu"), .9, {
        left: 0,
        ease: "Power4.easeOut",
        delay: .1
    }), TweenLite.to($$(".home", $$("#menu")), .5, {
        left: 0,
        ease: "Power4.easeOut",
        delay: .2
    }), TweenLite.to($$(".innovation", $$("#menu")), .5, {
        left: 0,
        ease: "Power4.easeOut",
        delay: .3
    }), TweenLite.to($$(".genetics", $$("#menu")), .5, {
        left: 0,
        ease: "Power4.easeOut",
        delay: .4
    }), TweenLite.to($$(".media", $$("#menu")), .5, {
        left: 0,
        ease: "Power4.easeOut",
        delay: .5
    }), TweenLite.to($$(".about", $$("#menu")), .5, {
        left: 0,
        ease: "Power4.easeOut",
        delay: .6
    }), TweenLite.to($$(".share", $$("#menu")), .5, {
        left: 0,
        ease: "Power4.easeOut",
        delay: .7
    }), TweenLite.to($$(".module_list"), 1.1, {
        right: 0,
        ease: Quint.easeOut,
        delay: .7
    }), TweenLite.to($$(".module_marquee"), .8, {
        opacity: 1,
        ease: Quint.easeOut,
        delay: 1.5
    }))
...
traskindustries.controller("AboutController", function ($scope, $http) {
    ...
    var setUpHomeSpace = function () {
        ...
        for (var mImageLoaded = function (e) {
            ...
            }, mPosters = $$(".poster", $$(".window", $$(".module_marquee"))), i = 0; i < mPosters.length; i++) mPosters[i].onload = mImageLoaded, mPosters[i].src = getAttr(mPosters[i], "ref");
        hasClass($$("mobile-menu"), "triggered") && ($$(".item", $$("#home"))[0].style.margin = "0 0 0 120px");
        var ytSel = ".yt_container",
            handleResizeHome = function () {
                for (var mPosters = $$(".poster", $$(".window", $$(".module_marquee"))), i = 0; i < mPosters.length; i++) centerImage(mPosters[i]);
                ...
                for (var videos = $$("video", $$(".window", $$(".module_marquee"))), vW = 1280, vH = 720, vR = vW / vH, winDim = getWinSize(), wR = winDim.width / winDim.height, i = 0; i < videos.length; i++) {
                    ...
                    } else t.style.height = "100%", t.style.width = "auto"
                }
            };
        ...
        for (var mVideos = $$(".video", $$(".module_marquee")), i = 0; i < mVideos.length; i++) {
            ...
            vine.bind($$(".icon-play", targ), "click", function (e) {
                var targ = e.srcElement || e.currentTarget || e,
                    _vidID = targ.parentNode.getAttribute ? targ.parentNode.getAttribute("rel") : targ.parentNode.attributes.rel.value;
                new YTPlayerWrapper({
                    container: $$(".yt_container", targ.parentNode).id,
                    vidID: _vidID,
                    attachToDOM: "true",
                    playingCallBack: Marq.pause
                }), targ.style.display = "none", $$(".poster", targ.parentNode).style.display = "none", $$(".title", targ.parentNode.parentNode.parentNode.parentNode).style.display = "none", $$(".body", targ.parentNode.parentNode.parentNode.parentNode).style.display = "none"
            })
        }
        for (var mHVideos = $$(".hosted_video", $$(".window", $$(".module_marquee"))), i = 0; i < mHVideos.length; i++) {
            var t = mHVideos[i] || mHVideos,
        ...
        for (var findOutHow = function (e) {
            ...
            dataLayer.push({
                pageName: "/FOH/" + nextSection,
                event: "pageView"
            }), hasClass(menuEl, "triggered") && (document.getElementById("menu").className = document.getElementById("menu").className.replace(/\bmobile-show\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className + " mobile-hide", menuEl.style.margin = "0", menuEl.className = menuEl.className.replace(/\btriggered\b/, "")), document.width > 1024 ? animateSectionOut(function () {
        ...
        var animateSectionOut = function (callback) {
            Marq.pause(), TweenLite.to($$(".module_marquee"), 1, {
                opacity: 0,
                ease: Quint.easeInOut,
                onComplete: callback
            });
            var targRight = -(.35 * getWinSize().width);
            1 == $$(".module_list").length && TweenLite.to($$(".module_list"), 1, {
                right: targRight,
                ease: Quint.easeInOut,
                delay: .2,
                onComplete: callback
    ...
    function fadeInItems() {
        for (var i = 0; i < $$(".item").length; i++) scheduleFadeIn(i)
    }
    ...
    function checkAutoPlay() {
        -1 != $location.$$url.indexOf("autoplay") && ytLightbox.openLightbox({
            currentTarget: $$(".video")
        })
    }
    ...
    function bindHoverstates(selector) {
        if (!$$(selector).length, $$(selector).length > 1)
            for (var i = 0; i < $$(selector).length; i++) vine.bind($$(selector)[i], "mouseover", hoverState), vine.bind($$(selector)[i], "mouseout", defaultState);
        else vine.bind($$(selector), "mouseover", hoverState), vine.bind($$(selector), "mouseout", defaultState)
    }
    ...
    function setItemSizes(selector, elWidth) {
        ...
        if ($$(selector).length > 1)
            for (var i = 0; i < $$(selector).length; i++) $$(selector)[i].style.width = elWidth;
        else $$(selector).style.width = elWidth
    }
    ...
    var msnry, windowWidth, baseWidth, lgWidth, currentDateTime, numCols, fadeDuration = 400,
        fadeDelay = 300,
        ytLightbox = {
            _inst: this,
            _vidID: null,
            ...
            openLightbox: function (e) {
                var targ = e.srcElement || e.currentTarget || e;
                if (ytLightbox._vidID = targ.parentNode ? $$(".video-block", targ.parentNode).getAttribute("rel") : targ, console.log($$(".video-block", targ.parentNode)), targ.parentNode) var eventName = $$("h2", targ.parentNode).innerHTML || "video clicked";
                dataLayer.push({
                    videoAction: "Play",
                    videoName: eventName,
                    event: "video"
            ...
            loadVideo: function () {
                ytLightbox._video = new YTPlayerWrapper({
                    container: "yt_lb_media",
                    vidID: ytLightbox._vidID
                })
            },
            ...
            init: function (selector) {
                if (!$$(selector).length, $$(selector).length > 1)
                    for (var i = 0; i < $$(selector).length; i++) vine.bind($$(selector)[i], "click", ytLightbox.openLightbox);
                else vine.bind($$(selector), "click", ytLightbox.openLightbox)
            }
        }
});
var menuInited = !1;
traskindustries.controller("MenuController", function ($scope, $http, $location) {
        ...
        function (d, s, id) {
            ...
            FB.init({
                ...
                dataLayer.push({
                    ...
                    socialActivity: "Like",
                    socialTarget: response,
                    event: "socialEvent"
                })
            }), FB.Event.subscribe("edge.remove", function (response) {
                dataLayer.push({
                    socialNetwork: "Facebook",
                    socialActivity: "Unlike",
                    socialTarget: response,
                    event: "socialEvent"
                })
    ...
    var nextSection, thisSection, setUpTopNav = function () {
            ...
            if (top_menu_items.length < 1) return window.setTimeout(setUpTopNav, 400), !1;
            thisSection = $location.$$path.split("/").join("") || "home";
            for (var i = 0; i < top_menu_items.length; i++) {
                var t = top_menu_items[i] || top_menu_items,
                    rel = t.getAttribute ? t.getAttribute("rel") : t.attributes.rel.value;
                if ("home" == rel && -1 == $location.$$path.indexOf("home") && vine.bind($$(".hitarea", t), "click", $scope.switchSection), 0 == $$(".icon", t).length && 1 == i) return window.setTimeout(setUpTopNav, 400), !1;
                if ($$(".icon", t).length) {
                    var spO = {
                        fps: 30,
                        cols: 15,
                        ...
                        cell_width: 70,
                        cell_height: 70,
                        src: "img/sprite_" + rel + ".png",
                        spriteElement: $$(".icon", t),
                        autoplay: !1,
                        loadCallBack: function () {},
                        animationCompleteCallback: function () {}
                    }, sp = new spriteAnimation(spO); - 1 == $location.$$path.indexOf(rel) ? (vine.bind($$(".hitarea", t), "click", $scope.switchSection), vine.bind($$(".hitarea", t), "mouseover", menuOver), vine.bind($$(".hitarea", t), "mouseout", menuOut)) : sp.goToFrame(sp.options.cols)
                }
            }
            ...
            for (var sob = $$(".social_ob"), i = 0; i < sob.length; i++) {
                ...
                vine.bind(t, "click", function (e) {
                    var targ = e.srcElement || e.currentTarget || e;
                    dataLayer.push({
                        eventLabel: targ.href,
                        event: "exitSite"
                    })
                })
            }
        }, menuOver = function (e) {
            var targ = e.srcElement || e.currentTarget,
                icon = $$(".icon", targ.parentNode);
    ...
    $scope.switchSection = function (e) {
        -1 != window.location.hash.indexOf("autoplay") && (window.location.hash = window.location.hash.split("autoplay").join("")), frameDoneList = [], frameReadyList = [], infoInitArray = [], $$(".module_marquee").length > 0 && $$(".module_marquee").Marq && $$(".module_marquee").Marq.pause();
        var targ = e.srcElement || e.currentTarget || e;
        if (nextSection = targ == e ? e : targ.parentNode.getAttribute ? targ.parentNode.getAttribute("rel") : targ.parentNode.attributes.rel.value, nextSection == thisSection || "home" == thisSection && "" == nextSection) return !1;
        ...
        "home" != thisSection && $$("." + thisSection, $$("#menu")).length > 0 && $$(".icon", $$("." + thisSection, $$("#menu"))).length > 0 && $$(".icon", $$("." + thisSection, $$("#menu"))).spriteAnimation.reverse("true", "true"), targ != e && "home" != nextSection && (vine.unbind(targ, "mouseout", menuOut), vine.unbind(targ, "mouseover", menuOver)), dataLayer.push({
            pageName: nextSection,
            event: "pageView"
        });
        var menuEl = document.getElementById("mobile-menu");
    ...
    var goToNextSection = function () {
        $location.path("/" + nextSection), $scope.$apply(), thisSection = nextSection, $$("." + thisSection, $$("#menu")).length > 0 && $$(".icon", $$("." + thisSection, $$("#menu"))).spriteAnimation && $$(".icon", $$("." + thisSection, $$("#menu"))).spriteAnimation.reverse("false", "true")
            ...
            TweenLite.to($$(".module_marquee"), 1, {
                opacity: 0,
                ease: Quint.easeInOut,
                onComplete: callback
            });
            var targRight = -(.35 * getWinSize().width);
            $$(".module_list") && 1 == $$(".module_list").length && TweenLite.to($$(".module_list"), 1, {
                right: targRight,
                ease: Quint.easeInOut,
                delay: .2,
                onComplete: callback
            }), $$("#masonry") && ($$("#legal").className += " hide", fade("out", $$("#masonry"), 450))
        };
    $scope.share = function ($event) {
        $event.stopPropagation();
        var shareEl = document.getElementById("share");
        hasClass(shareEl, "triggered") ? shareEl.className = "" : (shareEl.className = "triggered", dataLayer.push({
            eventCategory: "Site",
            eventAction: "View",
            eventLabel: "Share Container"
        }))
